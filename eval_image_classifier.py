...
tf.app.flags.DEFINE_string(
    'eval_dir', '/Users/Pharrell_WANG/fdc_train_dir/inception_v3/eval',
    'Directory where the results are saved to.')

tf.app.flags.DEFINE_string(
    'eval_interval', 600, 'eval interval in seconds.')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        batch_confusion = tf.confusion_matrix(labels, predictions, 37,
                                              name='batch_confusion')
        # tf.logging.info('confusion_matrix is coming.........')
        # tf.logging.info(batch_confusion)
        # confusion = tf.Variable(tf.zeros([37, 37], dtype=tf.int32),
        #                         name='confusion')
        ...
        # now = datetime.time()
        # np.savetxt(
        #     "/Users/Pharrell_WANG/workspace/models/slim/confusion/"
        #     + str(now) + "_confusion_matrix.csv",
        #     batch_confusion, fmt='%i',
        #     delimiter=",")
        ...
        config = tf.ConfigProto(device_count={
            'GPU': 0})  # mask GPUs visible to the session so it falls back on CPU
